"""
extract_gpt_embeddings.py
============================
Extracts per-layer hidden-state embeddings from the GPT-OSS-20B model.
...
Output
------
A folder `gpt_emb/` containing NUM_LAYERS files:
...
# ============================================================================
# 2.  LOAD GPT-OSS MODEL & TOKENIZER
# ============================================================================
# Change `MODEL_NAME` to whichever GPT-OSS checkpoint you have access to,
# or a local path.
...
gpt_model = AutoModelForCausalLM.from_pretrained(
    MODEL_NAME,
    torch_dtype="auto",          # let the model config decide the best dtype
...
NUM_LAYERS = gpt_model.config.num_hidden_layers
...
def get_all_layer_embeddings_from_generated(sentence: str, target_word: str):
    ...
    enc = {k: v.to(gpt_model.device) for k, v in enc.items()}
    ...
    attention_mask = torch.ones_like(generated_ids, device=gpt_model.device)
