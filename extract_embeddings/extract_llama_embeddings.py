"""
extract_llama_embeddings.py
============================
Extracts per-layer hidden-state embeddings from Llama-3.1-8B model.
...
--------------------
- sentences.csv   : columns  →  target word | sentence  (extracted from wikipedia or any other sources)
- concreteness.csv     : columns  →  Word | Conc.M          (Brysbaert et al.)

Output
------
A folder `llama_emb/` containing 32 files:
    layer_0.jsonl … layer_31.jsonl
...
# ============================================================================
# 2.  LOAD LLaMA-3 MODEL & TOKENIZER
# ============================================================================
# Change `model_name` to whichever LLaMA-3 checkpoint you have access to,
...
def get_all_layer_embeddings_from_generated(sentence: str, target_word: str):
    ...
    enc = {k: v.to(llama_model.device) for k, v in enc.items()}
    ...
    attention_mask = torch.ones_like(generated_ids, device=llama_model.device)
