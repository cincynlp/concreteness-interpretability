"""
extract_gemma_embeddings.py
============================
Extracts per-layer hidden-state embeddings from the Gemma-2-9B model.
...
Output
------
A folder `gemma_emb/` containing NUM_LAYERS files:
...
# ============================================================================
# 2.  LOAD GEMMA-2 MODEL & TOKENIZER
# ============================================================================
# Change `MODEL_NAME` to whichever Gemma-2 checkpoint you have access to,
# e.g. "google/gemma-2-9b-it" (instruction-tuned) or a local path.
...
NUM_LAYERS = gemma_model.config.num_hidden_layers
...
def get_all_layer_embeddings_from_generated(sentence: str, target_word: str):
    ...
    enc = {k: v.to(gemma_model.device) for k, v in enc.items()}
    ...
    attention_mask = torch.ones_like(generated_ids, device=gemma_model.device)
